...
centerFrame.Size = UDim2.new(0, 400, 0, 380)
centerFrame.Position = UDim2.new(0.5, -200, 0.5, -190)
...
keySubtitle.Text = "PREMIUM EDITION v4.5"
...
local function unlockMenu()
    if isValidKey(keyInput.Text) then
        keyGui:Destroy()
        createMainMenu()
        print("=== HDGB PREMIUM ULTRA MENU UNLOCKED ===")
        ...
        for i = 1, 5 do
            centerFrame.Position = UDim2.new(0.5, -200 + math.random(-5,5), 0.5, -190 + math.random(-3,3))
            task.wait(0.02)
        end
...
-- ============ CREATE MOON BACKGROUND FUNCTION ============
local function createVideoBackground(parent)
    local videoBg = Instance.new("Frame")
    videoBg.Size = UDim2.new(1, 0, 1, 0)
    videoBg.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
    videoBg.Parent = parent
    videoBg.ZIndex = 0
    ...
    skyGradient.Color = ColorSequence.new({
        ...
        ColorSequenceKeypoint.new(0.3, Color3.fromRGB(10, 8, 20)),
        ColorSequenceKeypoint.new(0.6, Color3.fromRGB(8, 6, 18)),
        ColorSequenceKeypoint.new(1, Color3.fromRGB(3, 3, 12))
    })
    skyGradient.Rotation = 45
    skyGradient.Parent = videoBg
    
    local moonContainer = Instance.new("Frame")
    moonContainer.Size = UDim2.new(0, 100, 0, 100)
    moonContainer.Position = UDim2.new(0.85, 0, 0.1, 0)
    moonContainer.BackgroundTransparency = 1
    moonContainer.Parent = videoBg
    moonContainer.ZIndex = 1
    
    local moonGlow = Instance.new("Frame")
    moonGlow.Size = UDim2.new(1, 20, 1, 20)
    moonGlow.Position = UDim2.new(0, -10, 0, -10)
    moonGlow.BackgroundColor3 = Color3.fromRGB(200, 200, 180)
    moonGlow.BackgroundTransparency = 0.85
    ...
    local moonBody = Instance.new("Frame")
    moonBody.Size = UDim2.new(1, 0, 1, 0)
    moonBody.BackgroundColor3 = Color3.fromRGB(230, 230, 210)
    moonBody.BackgroundTransparency = 0.15
    moonBody.BorderSizePixel = 0
    moonBody.Parent = moonContainer
    
    local moonBodyCorner = Instance.new("UICorner")
    moonBodyCorner.CornerRadius = UDim.new(1, 0)
    moonBodyCorner.Parent = moonBody
    
    for i = 1, 12 do
        local crater = Instance.new("Frame")
        crater.Size = UDim2.new(0, math.random(6, 15), 0, math.random(6, 15))
        crater.Position = UDim2.new(math.random(20, 80) / 100, 0, math.random(20, 80) / 100, 0)
        crater.BackgroundColor3 = Color3.fromRGB(160, 160, 140)
        crater.BackgroundTransparency = 0.5
        crater.BorderSizePixel = 0
        crater.Parent = moonBody
        
        local craterCorner = Instance.new("UICorner")
        craterCorner.CornerRadius = UDim.new(1, 0)
        craterCorner.Parent = crater
    end
    
    local moonFloat = game:GetService("TweenService"):Create(moonContainer, TweenInfo.new(8, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, -1, true), {
    ...
    for i = 1, 100 do
        ...
        local twinkle = game:GetService("TweenService"):Create(star, TweenInfo.new(math.random(2, 5), Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, -1, true), {
    ...
    for i = 1, 6 do
        ...
        cloud.Size = UDim2.new(0, math.random(100, 180), 0, math.random(30, 50))
        cloud.Position = UDim2.new(math.random(), 0, math.random(50, 85) / 100, 0)
        cloud.BackgroundColor3 = Color3.fromRGB(80, 80, 100)
        cloud.BackgroundTransparency = 0.8
        cloud.BorderSizePixel = 0
        cloud.Parent = videoBg
        cloud.ZIndex = 1
        
        local cloudCorner = Instance.new("UICorner")
        cloudCorner.CornerRadius = UDim.new(0, 20)
        cloudCorner.Parent = cloud
        
        local moveSpeed = math.random(30, 60)
        local moveCloud = game:GetService("TweenService"):Create(cloud, TweenInfo.new(moveSpeed, Enum.EasingStyle.Linear, Enum.EasingDirection.InOut, -1, true), {
            Position = UDim2.new(cloud.Position.X.Scale + 0.2, 0, cloud.Position.Y.Scale, 0)
        })
        moveCloud:Play()
    end
    ...
    local function createShootingStar()
        ...
        star.Position = UDim2.new(math.random(0, 50) / 100, 0, math.random(0, 30) / 100, 0)
        ...
        trail.Size = UDim2.new(0, 15, 0, 1)
        trail.Position = UDim2.new(0, -10, 0, 1)
        trail.BackgroundColor3 = Color3.fromRGB(255, 255, 200)
        trail.BackgroundTransparency = 0.5
        trail.BorderSizePixel = 0
        trail.Parent = star
        
        local moveStar = game:GetService("TweenService"):Create(star, TweenInfo.new(2, Enum.EasingStyle.Linear), {
            Position = UDim2.new(star.Position.X.Scale + 0.3, 0, star.Position.Y.Scale + 0.2, 0)
        })
        moveStar:Play()
        moveStar.Completed:Connect(function()
            star:Destroy()
        end)
    end
...
local updateInfo = {
    ...
    totalMods = #allMods,
    newMods = {
        "🔑 FLEXIBLE KEY SYSTEM - Key must start with D and end with F",
        "🖤 BLACK MENU - Perfect match with moon background",
        "🌙 MOON BACKGROUND - Animated moon & stars inside menu",
        "✨ 80+ WORKING MODS - All mods visible and working",
        "🎚️ 20+ ADJUSTABLE SLIDERS",
        "⭐ FAVORITES SYSTEM - Save your favorite mods",
        "🔍 SEARCH BAR - Find any mod instantly",
        "💀 MENU PERSISTS AFTER DEATH",
        "🖱️ DRAG ANYWHERE - Move the menu freely",
        "⌨️ KEYBOARD SHORTCUTS - Quick access to mods",
    },
    shortcuts = {
...
function createMainMenu()
    ...
    hoverName.Size = UDim2.new(0, 260, 0, 45)
    ...
    hoverName.Text = "💎 HDGB PREMIUM ULTRA 💎"
    ...
    hoverName.MouseEnter:Connect(function()
        game:GetService("TweenService"):Create(hoverName, TweenInfo.new(0.2), {BackgroundTransparency = 0, Size = UDim2.new(0, 270, 0, 50)}):Play()
    end)
    hoverName.MouseLeave:Connect(function()
        game:GetService("TweenService"):Create(hoverName, TweenInfo.new(0.2), {BackgroundTransparency = 0.1, Size = UDim2.new(0, 260, 0, 45)}):Play()
    end)
    ...
    -- Content frame (semi-transparent overlay)
    ...
    mainTitleText.Text = "HDGB PREMIUM ULTRA"
    ...
    mainSubText.Text = "Flexible Key Edition v4.5 | " .. #allMods .. " Mods"
    ...
    creditLabel.Text = "Created by " .. ownerName .. " | The Honored One | Flexible Key Edition"
    ...
    local function createUpdateTab()
        ...
        local sections = {
            ...
            {title="🔑 FLEXIBLE KEY SYSTEM", color=Color3.fromRGB(139,0,0), height=35, items=updateInfo.newMods},
        ...
        for _, section in ipairs(sections) do
            ...
            for i, item in ipairs(section.items) do
                if section.title ~= "📢 UPDATE " .. updateInfo.version then
                    ...
                    itemText.TextColor3 = section.title == "⌨️ KEYBOARD SHORTCUTS" and Color3.fromRGB(200,200,220) or Color3.fromRGB(100,200,255)
                    itemText.TextSize = 8
...
print("=== HDGB PREMIUM ULTRA v4.5 LOADED ===")
